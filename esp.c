...
int esp_inbound(struct rte_mbuf *m, struct ipsec_sa *sa, struct rte_crypto_op *cop)
{
    struct ip *ip4;
    ...
    payload_len = rte_pktmbuf_pkt_len(m) - ip_hdr_len - sizeof(struct rte_esp_hdr) - sa->iv_len -
                  sa->digest_len;
    ...
    if (sa->aead_algo == RTE_CRYPTO_AEAD_AES_GCM) {
        sym_cop->aead.data.offset = ip_hdr_len + sizeof(struct rte_esp_hdr) + sa->iv_len;
        ...
        sym_cop->aead.digest.data =
                rte_pktmbuf_mtod_offset(m, void *, rte_pktmbuf_pkt_len(m) - sa->digest_len);
        sym_cop->aead.digest.phys_addr =
                rte_pktmbuf_iova_offset(m, rte_pktmbuf_pkt_len(m) - sa->digest_len);
    } else {
        sym_cop->cipher.data.offset = ip_hdr_len + sizeof(struct rte_esp_hdr) + sa->iv_len;
        ...
        case RTE_CRYPTO_AUTH_SHA256_HMAC:
            ...
            sym_cop->auth.data.length = sizeof(struct rte_esp_hdr) + sa->iv_len + payload_len;
            break;
        default:
...
int esp_inbound_post(struct rte_mbuf *m, struct ipsec_sa *sa, struct rte_crypto_op *cop)
{
    struct ip *ip4, *ip;
    struct ip6_hdr *ip6;
    ...
    if (ips->type == RTE_SECURITY_ACTION_TYPE_INLINE_CRYPTO &&
        ...
        nexthdr =
                rte_pktmbuf_mtod_offset(m, uint8_t *, rte_pktmbuf_pkt_len(m) - sa->digest_len - 1);
        pad_len = nexthdr - 1;
    ...
    if (unlikely(IS_TRANSPORT(sa->flags))) {
        ...
        ip4 = (struct ip *)rte_pktmbuf_adj(m, sizeof(struct rte_esp_hdr) + sa->iv_len);
...
int esp_outbound(struct rte_mbuf *m, struct ipsec_sa *sa, struct rte_crypto_op *cop)
{
    struct ip *ip4;
    struct ip6_hdr *ip6;
    ...
    pad_payload_len = RTE_ALIGN_CEIL(rte_pktmbuf_pkt_len(m) - ip_hdr_len + 2, sa->block_size);
    ...
    if (unlikely(ip_hdr_len + sizeof(struct rte_esp_hdr) + sa->iv_len + pad_payload_len +
                         sa->digest_len >
                 IP_MAXPACKET)) {
    ...
    case IP4_TUNNEL:
        ip4 = ip4ip_outbound(m, sizeof(struct rte_esp_hdr) + sa->iv_len, &sa->src, &sa->dst);
        esp = (struct rte_esp_hdr *)(ip4 + 1);
        break;
    case IP6_TUNNEL:
        ip6 = ip6ip_outbound(m, sizeof(struct rte_esp_hdr) + sa->iv_len, &sa->src, &sa->dst);
        esp = (struct rte_esp_hdr *)(ip6 + 1);
        break;
    case TRANSPORT:
        new_ip = (uint8_t *)rte_pktmbuf_prepend(m, sizeof(struct rte_esp_hdr) + sa->iv_len);
    ...
    if (sa->aead_algo == RTE_CRYPTO_AEAD_AES_GCM) {
        ...
        sym_cop->aead.digest.data =
                rte_pktmbuf_mtod_offset(m, uint8_t *, rte_pktmbuf_pkt_len(m) - sa->digest_len);
        sym_cop->aead.digest.phys_addr =
                rte_pktmbuf_iova_offset(m, rte_pktmbuf_pkt_len(m) - sa->digest_len);
    } else {
        switch (sa->cipher_algo) {
        ...
        case RTE_CRYPTO_AUTH_SHA256_HMAC:
            sym_cop->auth.data.offset = ip_hdr_len;
            sym_cop->auth.data.length = sizeof(struct rte_esp_hdr) + sa->iv_len + pad_payload_len;
            break;
        default:
            RTE_LOG(ERR, IPSEC_ESP, "unsupported auth algorithm %u\n", sa->auth_algo);
            return -EINVAL;
...
int esp_outbound_post(struct rte_mbuf *m, struct ipsec_sa *sa, struct rte_crypto_op *cop)
{
