...
#define IPV4_DST_FROM_SP(acr) (rte_cpu_to_be_32((acr).field[DST_FIELD_IPV4].value.u32))

#define IPV4_SRC_FROM_SP(acr) (rte_cpu_to_be_32((acr).field[SRC_FIELD_IPV4].value.u32))
...
static struct rte_acl_field_def ip4_defs[NUM_FIELDS_IPV4] = {
    ...
    { .type = RTE_ACL_FIELD_TYPE_RANGE,
      ...
      .input_index = RTE_ACL_IPV4_PORTS,
      .offset = sizeof(struct ip) - offsetof(struct ip, ip_p) + sizeof(uint16_t) },
};
...
static int extend_sp_arr(struct acl4_rules **sp_tbl, uint32_t cur_cnt, uint32_t *cur_sz)
{
    if (*sp_tbl == NULL) {
...
void parse_sp4_tokens(char **tokens, uint32_t n_tokens, struct parse_status *status)
{
...
static void print_one_ip4_rule(const struct acl4_rules *rule, int32_t extra)
{
    ...
    printf("%hu : %hu %hu : %hu 0x%hhx/0x%hhx ", rule->field[SRCP_FIELD_IPV4].value.u16,
           rule->field[SRCP_FIELD_IPV4].mask_range.u16, rule->field[DSTP_FIELD_IPV4].value.u16,
           rule->field[DSTP_FIELD_IPV4].mask_range.u16, rule->field[PROTO_FIELD_IPV4].value.u8,
...
static inline void dump_ip4_rules(const struct acl4_rules *rule, int32_t num, int32_t extra)
{
    int32_t i;
...
static struct rte_acl_ctx *acl4_init(const char *name, int32_t socketid,
                                     const struct acl4_rules *rules, uint32_t rules_nb)
{
    char s[PATH_MAX];
...
void sp4_init(struct socket_ctx *ctx, int32_t socket_id)
{
    ...
    if (nb_acl4_rules_in > 0) {
        name = "sp_ip4_in";
        ctx->sp_ip4_in =
                (struct sp_ctx *)acl4_init(name, socket_id, acl4_rules_in, nb_acl4_rules_in);
    } else
    ...
    if (nb_acl4_rules_out > 0) {
        name = "sp_ip4_out";
        ctx->sp_ip4_out =
                (struct sp_ctx *)acl4_init(name, socket_id, acl4_rules_out, nb_acl4_rules_out);
    } else
        RTE_LOG(WARNING, IPSEC,
 ...
 */
int sp4_spi_present(uint32_t spi, int inbound, struct ip_addr ip_addr[2], uint32_t mask[2])
{
    uint32_t num;
