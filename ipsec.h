...
enum { IPSEC_SESSION_PRIMARY = 0, IPSEC_SESSION_FALLBACK = 1, IPSEC_SESSION_MAX };
 ...
 */
#define WITHOUT_TRANSPORT_VERSION(flags) ((flags) & (IP4_TUNNEL | IP6_TUNNEL | TRANSPORT))
...
static inline struct rte_ipsec_session *ipsec_get_primary_session(struct ipsec_sa *sa)
{
    return &sa->sessions[IPSEC_SESSION_PRIMARY];
}

static inline struct rte_ipsec_session *ipsec_get_fallback_session(struct ipsec_sa *sa)
{
    return &sa->sessions[IPSEC_SESSION_FALLBACK];
}

static inline enum rte_security_session_action_type ipsec_get_action_type(struct ipsec_sa *sa)
{
...
void inbound_sa_lookup(struct sa_ctx *sa_ctx, struct rte_mbuf *pkts[], void *sa[],
                       uint16_t nb_pkts);
