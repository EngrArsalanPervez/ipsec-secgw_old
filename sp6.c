...
#define IPV6_FROM_SP(acr, fidx_low, fidx_high) \
    (((uint64_t)(acr).field[(fidx_high)].value.u32 << 32) | (acr).field[(fidx_low)].value.u32)

#define IPV6_DST_FROM_SP(addr, acr)                                                       \
    do {                                                                                  \
        (addr).ip.ip6.ip6[0] = rte_cpu_to_be_64(IPV6_FROM_SP((acr), IP6_DST1, IP6_DST0)); \
        (addr).ip.ip6.ip6[1] = rte_cpu_to_be_64(IPV6_FROM_SP((acr), IP6_DST3, IP6_DST2)); \
    } while (0)

#define IPV6_SRC_FROM_SP(addr, acr)                                                       \
    do {                                                                                  \
        (addr).ip.ip6.ip6[0] = rte_cpu_to_be_64(IPV6_FROM_SP((acr), IP6_SRC1, IP6_SRC0)); \
        (addr).ip.ip6.ip6[1] = rte_cpu_to_be_64(IPV6_FROM_SP((acr), IP6_SRC3, IP6_SRC2)); \
    } while (0)

#define IPV6_DST_MASK_FROM_SP(mask, acr)                                                    \
    ((mask) = (acr).field[IP6_DST0].mask_range.u32 + (acr).field[IP6_DST1].mask_range.u32 + \
              (acr).field[IP6_DST2].mask_range.u32 + (acr).field[IP6_DST3].mask_range.u32)

#define IPV6_SRC_MASK_FROM_SP(mask, acr)                                                    \
    ((mask) = (acr).field[IP6_SRC0].mask_range.u32 + (acr).field[IP6_SRC1].mask_range.u32 + \
              (acr).field[IP6_SRC2].mask_range.u32 + (acr).field[IP6_SRC3].mask_range.u32)
...
static int extend_sp_arr(struct acl6_rules **sp_tbl, uint32_t cur_cnt, uint32_t *cur_sz)
{
    if (*sp_tbl == NULL) {
...
void parse_sp6_tokens(char **tokens, uint32_t n_tokens, struct parse_status *status)
{
    ...
    for (ti = 2; ti < n_tokens; ti++) {
        ...
        if (strcmp(tokens[ti], "protect") == 0) {
            ...
            tv = atoi(tokens[ti]);
            APP_CHECK(tv != DISCARD && tv != BYPASS, status, "invalid SPI: %s", tokens[ti]);
            if (status->status < 0)
                return;
        ...
        if (strcmp(tokens[ti], "src") == 0) {
            ...
            rule_ipv6->field[1].value.u32 = (uint32_t)ip.s6_addr[0] << 24 |
            ...
            rule_ipv6->field[2].value.u32 = (uint32_t)ip.s6_addr[4] << 24 |
        ...
        if (strcmp(tokens[ti], "dst") == 0) {
            ...
            rule_ipv6->field[5].value.u32 = (uint32_t)ip.s6_addr[0] << 24 |
                                            (uint32_t)ip.s6_addr[1] << 16 |
                                            (uint32_t)ip.s6_addr[2] << 8 | (uint32_t)ip.s6_addr[3];
            rule_ipv6->field[5].mask_range.u32 = (depth > 32) ? 32 : depth;
            depth = (depth > 32) ? (depth - 32) : 0;
            rule_ipv6->field[6].value.u32 = (uint32_t)ip.s6_addr[4] << 24 |
                                            (uint32_t)ip.s6_addr[5] << 16 |
                                            (uint32_t)ip.s6_addr[6] << 8 | (uint32_t)ip.s6_addr[7];
...
static inline void print_one_ip6_rule(const struct acl6_rules *rule, int32_t extra)
{
    uint8_t a, b, c, d;
    ...
    printf(":%.2x%.2x:%.2x%.2x/%u ", a, b, c, d,
           rule->field[IP6_SRC0].mask_range.u32 + rule->field[IP6_SRC1].mask_range.u32 +
                   rule->field[IP6_SRC2].mask_range.u32 + rule->field[IP6_SRC3].mask_range.u32);
    ...
    printf(":%.2x%.2x:%.2x%.2x/%u ", a, b, c, d,
           rule->field[IP6_DST0].mask_range.u32 + rule->field[IP6_DST1].mask_range.u32 +
                   rule->field[IP6_DST2].mask_range.u32 + rule->field[IP6_DST3].mask_range.u32);
...
static inline void dump_ip6_rules(const struct acl6_rules *rule, int32_t num, int32_t extra)
{
    int32_t i;
...
static struct rte_acl_ctx *acl6_init(const char *name, int32_t socketid,
                                     const struct acl6_rules *rules, uint32_t rules_nb)
{
    char s[PATH_MAX];
...
void sp6_init(struct socket_ctx *ctx, int32_t socket_id)
{
    ...
    if (nb_acl6_rules_in > 0) {
        name = "sp_ip6_in";
        ctx->sp_ip6_in =
                (struct sp_ctx *)acl6_init(name, socket_id, acl6_rules_in, nb_acl6_rules_in);
    } else
    ...
    if (nb_acl6_rules_out > 0) {
        name = "sp_ip6_out";
        ctx->sp_ip6_out =
                (struct sp_ctx *)acl6_init(name, socket_id, acl6_rules_out, nb_acl6_rules_out);
    } else
        RTE_LOG(WARNING, IPSEC,
 ...
 */
int sp6_spi_present(uint32_t spi, int inbound, struct ip_addr ip_addr[2], uint32_t mask[2])
{
    uint32_t num;
