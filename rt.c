...
void parse_rt_tokens(char **tokens, uint32_t n_tokens, struct parse_status *status)
{
    ...
    if (strcmp(tokens[0], "ipv4") == 0) {
        ...
        APP_CHECK(*n_rts <= RT_IPV4_MAX_RULES - 1, status, "too many rt rules, abort insertion\n");
        if (status->status < 0)
            return;
        ...
        APP_CHECK(*n_rts <= RT_IPV6_MAX_RULES - 1, status, "too many rt rules, abort insertion\n");
        if (status->status < 0)
            return;
    } else {
...
void rt_init(struct socket_ctx *ctx, int32_t socket_id)
{
    ...
    for (i = 0; i < nb_rt_ip6; i++) {
        ...
        printf("LPM6: Adding route "
               ...
               (uint16_t)((rt_ip6[i].ip[14] << 8) | rt_ip6[i].ip[15]), rt_ip6[i].depth,
               rt_ip6[i].if_out);
    }
