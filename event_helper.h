...
#define EVENT_MODE_MAX_LCORE_LINKS (EVENT_MODE_MAX_EVENT_DEVS * EVENT_MODE_MAX_EVENT_QUEUES_PER_DEV)
 ...
 */
enum eh_ipsec_mode_types { EH_IPSEC_MODE_TYPE_APP = 0, EH_IPSEC_MODE_TYPE_DRIVER };
...
struct rx_adapter_conf {
    ...
    struct rx_adapter_connection_info conn[EVENT_MODE_MAX_CONNECTIONS_PER_ADAPTER];
};
...
struct tx_adapter_conf {
    ...
    uint32_t tx_core_id;
    uint8_t nb_connections;
    struct tx_adapter_connection_info conn[EVENT_MODE_MAX_CONNECTIONS_PER_TX_ADAPTER];
    uint8_t tx_ev_queue;
};
