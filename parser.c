...
static int parse_tokenize_string(char *string, char *tokens[], uint32_t *n_tokens)
{
    uint32_t i;
...
int parse_ipv4_addr(const char *token, struct in_addr *ipv4, uint32_t *mask)
{
    ...
    pch = strchr(token, '/');
    if (pch != NULL) {
        strlcpy(ip_str, token, RTE_MIN((unsigned int long)(pch - token + 1), sizeof(ip_str)));
        pch += 1;
        if (is_str_num(pch) != 0)
            return -EINVAL;
        if (mask)
...
static void cfg_sp_add_cfg_item_parsed(void *parsed_result, __rte_unused struct cmdline *cl,
                                       void *data)
{
    ...
    APP_CHECK((parse_tokenize_string(params->multi_string, tokens, &n_tokens) == 0), status,
              "too many arguments");
...
static cmdline_parse_token_string_t cfg_sp_add_multi_str =
        TOKEN_STRING_INITIALIZER(struct cfg_sp_add_cfg_item, multi_string, TOKEN_STRING_MULTI);
...
static void cfg_sa_add_cfg_item_parsed(void *parsed_result, __rte_unused struct cmdline *cl,
                                       void *data)
{
...
static cmdline_parse_token_string_t cfg_sa_add_multi_str =
        TOKEN_STRING_INITIALIZER(struct cfg_sa_add_cfg_item, multi_string, TOKEN_STRING_MULTI);
...
static void cfg_rt_add_cfg_item_parsed(void *parsed_result, __rte_unused struct cmdline *cl,
                                       void *data)
{
...
static cmdline_parse_token_string_t cfg_rt_add_multi_str =
        TOKEN_STRING_INITIALIZER(struct cfg_rt_add_cfg_item, multi_string, TOKEN_STRING_MULTI);
...
static void cfg_flow_add_cfg_item_parsed(void *parsed_result, __rte_unused struct cmdline *cl,
                                         void *data)
{
...
static cmdline_parse_token_string_t cfg_flow_add_flow_str =
        TOKEN_STRING_INITIALIZER(struct cfg_flow_add_cfg_item, flow_keyword, "flow");

static cmdline_parse_token_string_t cfg_flow_add_multi_str =
        TOKEN_STRING_INITIALIZER(struct cfg_flow_add_cfg_item, multi_string, TOKEN_STRING_MULTI);
...
static void cfg_parse_neigh(void *parsed_result, __rte_unused struct cmdline *cl, void *data)
{
    int32_t rc;
...
cmdline_parse_ctx_t ipsec_ctx[] = {
    (cmdline_parse_inst_t *)&cfg_sp_add_rule,    (cmdline_parse_inst_t *)&cfg_sa_add_rule,
    (cmdline_parse_inst_t *)&cfg_rt_add_rule,    (cmdline_parse_inst_t *)&cfg_flow_add_rule,
    (cmdline_parse_inst_t *)&cfg_neigh_add_rule, NULL,
};
