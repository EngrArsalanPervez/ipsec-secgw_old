...
static inline unsigned int eh_get_next_eth_core(struct eventmode_conf *em_conf)
{
...
static inline unsigned int eh_get_next_active_core(struct eventmode_conf *em_conf,
...
static struct eventdev_params *eh_get_eventdev_params(struct eventmode_conf *em_conf,
                                                      uint8_t eventdev_id)
{
    int i;
...
static void eh_do_capability_check(struct eventmode_conf *em_conf)
{
    ...
    for (i = 0; i < em_conf->nb_eventdev; i++) {
        ...
        if (eh_dev_has_rx_internal_port(eventdev_id) && eh_dev_has_tx_internal_port(eventdev_id)) {
...
static int eh_set_default_conf_rx_adapter(struct eventmode_conf *em_conf)
{
	 ...
	 */
    nb_eventqueue = eventdev_config->all_internal_ports ? eventdev_config->nb_eventqueue :
...
static int eh_initialize_eventdev(struct eventmode_conf *em_conf)
{
    ...
    for (i = 0; i < nb_eventdev; i++) {
        ...
        eventdev_conf.nb_event_queue_flows = evdev_default_conf.max_event_queue_flows;
        eventdev_conf.nb_event_port_dequeue_depth = evdev_default_conf.max_event_port_dequeue_depth;
        eventdev_conf.nb_event_port_enqueue_depth = evdev_default_conf.max_event_port_enqueue_depth;
...
static int eh_event_vector_limits_validate(struct eventmode_conf *em_conf, uint8_t ev_dev_id,
                                           uint8_t ethdev_id)
{
    ...
    ret = rte_event_eth_rx_adapter_vector_limits_get(ev_dev_id, ethdev_id, &limits);
    if (ret) {
    ...
    if (vector_size < limits.min_sz || vector_size > limits.max_sz) {
        EH_LOG_ERR("Vector size [%d] not within limits min[%d] max[%d]", vector_size, limits.min_sz,
                   limits.max_sz);
        return -EINVAL;
    }
    ...
    if (em_conf->vector_tmo_ns > limits.max_timeout_ns ||
        ...
        EH_LOG_ERR("Vector timeout [%" PRIu64 "] not within limits max[%" PRIu64 "] min[%" PRIu64
                   "]",
                   em_conf->vector_tmo_ns, limits.max_timeout_ns, limits.min_timeout_ns);
        return -EINVAL;
    }
    return 0;
}

static int eh_rx_adapter_configure(struct eventmode_conf *em_conf, struct rx_adapter_conf *adapter)
{
    ...
    if (em_conf->ext_params.event_vector) {
        ...
        vector_pool = rte_event_vector_pool_create("vector_pool", nb_elem, 0,
    ...
    /* Create Rx adapter */
    ret = rte_event_eth_rx_adapter_create(adapter->adapter_id, adapter->eventdev_id, &port_conf);
    if (ret < 0) {
    ...
    for (j = 0; j < adapter->nb_connections; j++) {
        ...
        if (em_conf->ext_params.event_vector) {
            ret = eh_event_vector_limits_validate(em_conf, eventdev_id, conn->ethdev_id);
            if (ret)
                return ret;
        ...
        ret = rte_event_eth_rx_adapter_queue_add(adapter->adapter_id, conn->ethdev_id,
                                                 conn->ethdev_rx_qid, &queue_conf);
        if (ret < 0) {
    ...
    ret = rte_event_eth_rx_adapter_service_id_get(adapter->adapter_id, &service_id);
    if (ret != -ESRCH && ret < 0) {
...
static int32_t eh_start_worker_eth_core(struct eventmode_conf *conf, uint32_t lcore_id)
{
    ...
    for (i = 0; i < conf->nb_rx_adapter; i++) {
        ...
        ret = rte_event_eth_rx_adapter_service_id_get(adapter_id, &(service_id[service_count]));
    ...
    for (i = 0; i < conf->nb_tx_adapter; i++) {
        ...
        /* Get the service ID for the adapters */
        ret = rte_event_eth_tx_adapter_service_id_get(adapter_id, &(service_id[service_count]));
...
static struct eh_app_worker_params *eh_find_worker(uint32_t lcore_id, struct eh_conf *conf,
                                                   struct eh_app_worker_params *app_wrkrs,
...
static int eh_tx_adapter_configure(struct eventmode_conf *em_conf, struct tx_adapter_conf *adapter)
{
    struct rte_event_dev_info evdev_default_conf = { 0 };
    struct rte_event_port_conf port_conf = { 0 };
    struct tx_adapter_connection_info *conn;
    ...
    /* Create adapter */
    ret = rte_event_eth_tx_adapter_create(adapter->adapter_id, adapter->eventdev_id, &port_conf);
    if (ret < 0) {
    ...
    for (j = 0; j < adapter->nb_connections; j++) {
        ...
        /* Add queue to the adapter */
        ret = rte_event_eth_tx_adapter_queue_add(adapter->adapter_id, conn->ethdev_id,
                                                 conn->ethdev_tx_qid);
        if (ret < 0) {
    ...
    ret = rte_event_eth_tx_adapter_event_port_get(adapter->adapter_id, &tx_port_id);
    if (ret) {
    ...
    ret = rte_event_port_link(eventdev_id, tx_port_id, &(adapter->tx_ev_queue), NULL, 1);
    if (ret != 1) {
    ...
    ret = rte_event_eth_tx_adapter_service_id_get(adapter->adapter_id, &service_id);
    if (ret != -ESRCH && ret < 0) {
...
static void eh_display_operating_mode(struct eventmode_conf *em_conf)
{
    ...
    EH_LOG_INFO("\tScheduling type: \t%s", sched_types[em_conf->ext_params.sched_type]);
...
static void eh_display_event_dev_conf(struct eventmode_conf *em_conf)
{
    ...
    for (i = 0; i < em_conf->nb_eventdev; i++) {
        sprintf(print_buf, "\tDev ID: %-2d \tQueues: %-2d \tPorts: %-2d",
                em_conf->eventdev_config[i].eventdev_id, em_conf->eventdev_config[i].nb_eventqueue,
...
static void eh_display_rx_adapter_conf(struct eventmode_conf *em_conf)
{
    ...
    for (i = 0; i < nb_rx_adapter; i++) {
        adapter = &(em_conf->rx_adapter[i]);
        sprintf(print_buf, "\tRx adapter ID: %-2d\tConnections: %-2d\tEvent dev ID: %-2d",
                adapter->adapter_id, adapter->nb_connections, adapter->eventdev_id);
        if (adapter->rx_core_id == (uint32_t)-1)
            sprintf(print_buf + strlen(print_buf), "\tRx core: %-2s", "[INTERNAL PORT]");
        else if (adapter->rx_core_id == RTE_MAX_LCORE)
            sprintf(print_buf + strlen(print_buf), "\tRx core: %-2s", "[NONE]");
        else
            sprintf(print_buf + strlen(print_buf), "\tRx core: %-2d", adapter->rx_core_id);
        ...
        for (j = 0; j < adapter->nb_connections; j++) {
            ...
            if (conn->ethdev_rx_qid == -1)
                ...
                sprintf(print_buf + strlen(print_buf), "\tEth rx queue: %-2d", conn->ethdev_rx_qid);

            sprintf(print_buf + strlen(print_buf), "\tEvent queue: %-2d", conn->eventq_id);
            EH_LOG_INFO("%s", print_buf);
        }
    }
    EH_LOG_INFO("");
...
static void eh_display_tx_adapter_conf(struct eventmode_conf *em_conf)
{
    ...
    for (i = 0; i < nb_tx_adapter; i++) {
        adapter = &(em_conf->tx_adapter[i]);
        sprintf(print_buf, "\tTx adapter ID: %-2d\tConnections: %-2d\tEvent dev ID: %-2d",
                adapter->adapter_id, adapter->nb_connections, adapter->eventdev_id);
        if (adapter->tx_core_id == (uint32_t)-1)
            sprintf(print_buf + strlen(print_buf), "\tTx core: %-2s", "[INTERNAL PORT]");
        else if (adapter->tx_core_id == RTE_MAX_LCORE)
            sprintf(print_buf + strlen(print_buf), "\tTx core: %-2s", "[NONE]");
        else
            sprintf(print_buf + strlen(print_buf), "\tTx core: %-2d,\tInput event queue: %-2d",
        ...
        for (j = 0; j < adapter->nb_connections; j++) {
            ...
            if (conn->ethdev_tx_qid == -1)
                ...
                sprintf(print_buf + strlen(print_buf), "\tEth tx queue: %-2d", conn->ethdev_tx_qid);
            EH_LOG_INFO("%s", print_buf);
        }
    }
    EH_LOG_INFO("");
...
static void eh_display_link_conf(struct eventmode_conf *em_conf)
{
    ...
    for (i = 0; i < em_conf->nb_link; i++) {
        ...
        sprintf(print_buf, "\tEvent dev ID: %-2d\tEvent port: %-2d", link->eventdev_id,
                link->event_port_id);

        if (em_conf->ext_params.all_ev_queue_to_ev_port)
            sprintf(print_buf + strlen(print_buf), "Event queue: %-2s\t", "ALL");
        else
            sprintf(print_buf + strlen(print_buf), "Event queue: %-2d\t", link->eventq_id);
...
struct eh_conf *eh_conf_init(void)
{
    ...
    bitmap = rte_zmalloc("event-helper-ethcore-bitmap", nb_bytes, RTE_CACHE_LINE_SIZE);
    if (!bitmap) {
...
int32_t eh_devs_uninit(struct eh_conf *conf)
{
    ...
    for (i = 0; i < em_conf->nb_rx_adapter; i++) {
        ...
        for (j = 0; j < em_conf->rx_adapter[i].nb_connections; j++) {
            ret = rte_event_eth_rx_adapter_queue_del(id, em_conf->rx_adapter[i].conn[j].ethdev_id,
                                                     -1);
            if (ret < 0) {
    ...
    for (i = 0; i < em_conf->nb_tx_adapter; i++) {
        ...
        for (j = 0; j < em_conf->tx_adapter[i].nb_connections; j++) {
            ret = rte_event_eth_tx_adapter_queue_del(id, em_conf->tx_adapter[i].conn[j].ethdev_id,
                                                     -1);
            if (ret < 0) {
