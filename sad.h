...
static inline int cmp_sa_key(struct ipsec_sa *sa, int is_v4, struct rte_ipv4_hdr *ipv4,
                             struct rte_ipv6_hdr *ipv6)
{
    ...
    if ((sa_type == TRANSPORT) ||
        ...
        /* IPv6 check */
        (!is_v4 && (sa_type == IP6_TUNNEL) && (!memcmp(sa->src.ip.ip6.ip6, ipv6->src_addr, 16)) &&
...
static inline void sa_cache_update(struct ipsec_sa **sa_cache, struct ipsec_sa *sa, uint32_t mask)
{
    uint32_t cache_idx;
...
static inline void sad_lookup(struct ipsec_sad *sad, struct rte_mbuf *pkts[], void *sa[],
                              uint16_t nb_pkts)
{
    uint32_t i;
    ...
    for (i = 0; i < nb_pkts; i++) {
        ...
        esp = rte_pktmbuf_mtod_offset(pkts[i], struct rte_esp_hdr *, pkts[i]->l3_len + udp_hdr_len);
