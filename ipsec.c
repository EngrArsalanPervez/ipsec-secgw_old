...
static inline void set_ipsec_conf(struct ipsec_sa *sa, struct rte_security_ipsec_xform *ipsec)
{
    if (ipsec->mode == RTE_SECURITY_IPSEC_SA_MODE_TUNNEL) {
        ...
        if (IS_IP4_TUNNEL(sa->flags)) {
            ...
            memcpy((uint8_t *)&tunnel->ipv6.src_addr, (uint8_t *)&sa->src.ip.ip6.ip6_b, 16);

            memcpy((uint8_t *)&tunnel->ipv6.dst_addr, (uint8_t *)&sa->dst.ip.ip6.ip6_b, 16);
        }
                             ...
                             struct rte_ipsec_session *ips)
{
    ...
    RTE_LOG_DP(DEBUG, IPSEC,
               ...
               "%u qp %u\n",
               sa->spi, ipsec_ctx->tbl[cdev_id_qp].id, ipsec_ctx->tbl[cdev_id_qp].qp);
    ...
    if (ips->type != RTE_SECURITY_ACTION_TYPE_NONE &&
        ...
        if (ips->type == RTE_SECURITY_ACTION_TYPE_LOOKASIDE_PROTOCOL) {
            struct rte_security_ctx *ctx = (struct rte_security_ctx *)rte_cryptodev_get_sec_ctx(
            ...
            ips->security.ses = rte_security_session_create(
                    ctx, &sess_conf, ipsec_ctx->session_pool, ipsec_ctx->session_priv_pool);
        ...
        rte_cryptodev_sym_session_init(ipsec_ctx->tbl[cdev_id_qp].id, ips->crypto.ses, sa->xforms,
                          ...
                          struct rte_ipsec_session *ips)
{
    ...
    if (IS_TRANSPORT(sa->flags)) {
        ...
        if (IS_IP4(sa->flags)) {
            ...
            memcpy(sess_conf.ipsec.tunnel.ipv6.src_addr.s6_addr, sa->src.ip.ip6.ip6_b, 16);
            memcpy(sess_conf.ipsec.tunnel.ipv6.dst_addr.s6_addr, sa->dst.ip.ip6.ip6_b, 16);
        }
    ...
    if (ips->type == RTE_SECURITY_ACTION_TYPE_INLINE_CRYPTO) {
        ...
        ips->security.ses = rte_security_session_create(sec_ctx, &sess_conf, skt_ctx->session_pool,
        ...
        while (sec_cap->action != RTE_SECURITY_ACTION_TYPE_NONE) {
            if (sec_cap->action == ips->type && sec_cap->protocol == RTE_SECURITY_PROTOCOL_IPSEC &&
...
flow_create:
        sa->flow = rte_flow_create(sa->portid, &sa->attr, sa->pattern, sa->action, &err);
        if (sa->flow == NULL) {
flow_create_failure:
        ...
        if (sec_ctx == NULL) {
            RTE_LOG(ERR, IPSEC, "Ethernet device doesn't have security features registered\n");
            return -1;
        }
...
int create_ipsec_esp_flow(struct ipsec_sa *sa)
{
    ...
    if (IS_IP6(sa->flags)) {
        ...
        sa->pattern[1].type = RTE_FLOW_ITEM_TYPE_IPV6;
        sa->pattern[1].spec = &sa->ipv6_spec;
        memcpy(sa->ipv6_spec.hdr.dst_addr, sa->dst.ip.ip6.ip6_b, sizeof(sa->dst.ip.ip6.ip6_b));
        memcpy(sa->ipv6_spec.hdr.src_addr, sa->src.ip.ip6.ip6_b, sizeof(sa->src.ip.ip6.ip6_b));
        sa->pattern[2].type = RTE_FLOW_ITEM_TYPE_ESP;
        sa->pattern[2].spec = &sa->esp_spec;
        sa->pattern[2].mask = &rte_flow_item_esp_mask;
        sa->esp_spec.hdr.spi = rte_cpu_to_be_32(sa->spi);
...
static inline void ipsec_enqueue(ipsec_xform_fn xform_func, struct ipsec_ctx *ipsec_ctx,
                                 struct rte_mbuf *pkts[], void *sas[], uint16_t nb_pkts)
{
    int32_t ret = 0, i;
    ...
    for (i = 0; i < nb_pkts; i++) {
        ...
        case RTE_SECURITY_ACTION_TYPE_LOOKASIDE_PROTOCOL:
            ...
            if (unlikely((pkts[i]->packet_type & (RTE_PTYPE_TUNNEL_MASK | RTE_PTYPE_L4_MASK)) ==
        ...
        case RTE_SECURITY_ACTION_TYPE_INLINE_PROTOCOL:
            ...
            if (ips->security.ol_flags & RTE_SECURITY_TX_OLOAD_NEED_MDATA)
                rte_security_set_pkt_metadata(ips->security.ctx, ips->security.ses, pkts[i], NULL);
            continue;
...
static inline int32_t ipsec_inline_dequeue(ipsec_xform_fn xform_func, struct ipsec_ctx *ipsec_ctx,
...
static inline int ipsec_dequeue(ipsec_xform_fn xform_func, struct ipsec_ctx *ipsec_ctx,
                                struct rte_mbuf *pkts[], uint16_t max_pkts)
{
    ...
    for (i = 0; i < ipsec_ctx->nb_qps && nb_pkts < max_pkts; i++) {
        ...
        nb_cops = rte_cryptodev_dequeue_burst(cqp->id, cqp->qp, cops, max_pkts - nb_pkts);
        ...
        for (j = 0; j < nb_cops; j++) {
            ...
            } else if (ipsec_get_action_type(sa) == RTE_SECURITY_ACTION_TYPE_LOOKASIDE_PROTOCOL) {
                if (cops[j]->status) {
                    free_pkts(&pkt, 1);
                    continue;
                }
...
uint16_t ipsec_inbound(struct ipsec_ctx *ctx, struct rte_mbuf *pkts[], uint16_t nb_pkts,
                       uint16_t len)
{
    void *sas[nb_pkts];
...
uint16_t ipsec_inbound_cqp_dequeue(struct ipsec_ctx *ctx, struct rte_mbuf *pkts[], uint16_t len)
{
    return ipsec_dequeue(esp_inbound_post, ctx, pkts, len);
}

uint16_t ipsec_outbound(struct ipsec_ctx *ctx, struct rte_mbuf *pkts[], uint32_t sa_idx[],
...
uint16_t ipsec_outbound_cqp_dequeue(struct ipsec_ctx *ctx, struct rte_mbuf *pkts[], uint16_t len)
{
