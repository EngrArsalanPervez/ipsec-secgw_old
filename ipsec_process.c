...
static inline void enqueue_cop_bulk(struct cdev_qp *cqp, struct rte_crypto_op *cop[], uint32_t num)
{
    uint32_t i, k, len, n;
...
static inline int fill_ipsec_session(struct rte_ipsec_session *ss, struct ipsec_ctx *ctx,
                                     struct ipsec_sa *sa)
{
    int32_t rc;
 ...
 */
static uint32_t sa_group(void *sa_ptr[], struct rte_mbuf *pkts[], struct rte_ipsec_group grp[],
                         uint32_t num)
{
    uint32_t i, n, spi;
    void *sa;
 ...
 */
static inline void copy_to_trf(struct ipsec_traffic *trf, uint64_t satp, struct rte_mbuf *mb[],
                               uint32_t num)
{
    uint32_t j, ofs, s;
...
static uint32_t ipsec_prepare_crypto_group(struct ipsec_ctx *ctx, struct ipsec_sa *sa,
 ...
 * Should be removed in future.
 */
static inline void prep_process_group(void *sa, struct rte_mbuf *mb[], uint32_t cnt)
{
    uint32_t j;
    struct ipsec_mbuf_metadata *priv;
 ...
 */
static uint32_t ipsec_process_inline_group(struct rte_ipsec_session *ips, void *sa,
                                           struct ipsec_traffic *trf, struct rte_mbuf *mb[],
                                           uint32_t cnt)
{
    uint64_t satp;
    uint32_t k;
...
void ipsec_process(struct ipsec_ctx *ctx, struct ipsec_traffic *trf)
{
    ...
    for (i = 0; i != n; i++) {
		 ...
		 */
        if (sa != NULL)
            ips = (pg->id.val & IPSEC_SA_OFFLOAD_FALLBACK_FLAG) ? ipsec_get_fallback_session(sa) :
                                                                  ipsec_get_primary_session(sa);

        /* no valid HW session for that SA, try to create one */
        if (sa == NULL || (ips->crypto.ses == NULL && fill_ipsec_session(ips, ctx, sa) != 0))
            k = 0;
...
static inline uint32_t cqp_dequeue(struct cdev_qp *cqp, struct rte_crypto_op *cop[], uint32_t num)
{
    uint32_t n;
...
static inline uint32_t ctx_dequeue(struct ipsec_ctx *ctx, struct rte_crypto_op *cop[], uint32_t num)
{
    uint32_t i, n;
...
void ipsec_cqp_process(struct ipsec_ctx *ctx, struct ipsec_traffic *trf)
{
    ...
    ng = rte_ipsec_pkt_crypto_group((const struct rte_crypto_op **)(uintptr_t)cop, out->pkts, grp,
                                    n);
